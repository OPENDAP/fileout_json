...
vector<FoJsonDim *> FoJsonArray::Dimensions;

/** @brief Constructor for FoJsonArray that takes a DAP Array
 *
 ...
 */
FoJsonArray::FoJsonArray(BaseType *b) :
		FoJsonBaseType(), _a(0), _array_type(NC_NAT), _ndims(0), _actual_ndims(0), _nelements(1), _dim_ids(0), _dim_sizes(
				0), _str_data(0), _dont_use_it(false), _chunksizes(0), _grid_maps(0)
{
	_a = dynamic_cast<Array *>(b);
	if (!_a) {
		string s = (string) "File out netcdf, FoJsonArray was passed a " + "variable that is not a DAP Array";
 ...
 * list. Since the dimensions can be shared by other arrays, FoJsonDim
 * uses reference counting, so the instances aren't actually deleted
 * here, but their reference count is decremented
 *
 * The DAP Array instance does not belong to the FoJsonArray instance, so
 * it is not deleted.
 */
FoJsonArray::~FoJsonArray()
{
#if 0
	// No need to use erase. jhrg 8/28/13
	bool done = false;
	while (!done) {
		vector<FoJsonDim *>::iterator i = _dims.begin();
		vector<FoJsonDim *>::iterator e = _dims.end();
		if (i == e) {
			done = true;
		}
		else {
	...
	// Added jhrg 8/28/13
	vector<FoJsonDim*>::iterator d = _dims.begin();
	while (d != _dims.end()) {
		(*d)->decref();
		++d;
	}
	...
	// Added jhrg 8/28/13
	vector<FoJsonMap*>::iterator i = _grid_maps.begin();
...
/** @brief Converts the DAP Array to a FoJsonArray
 *
 * Does this by converting the name to a valid netcdf variable name,
 * creating FoJsonDim instances for each of the dimensions of the array,
 ...
 */
void FoJsonArray::convert(vector<string> embed)
{
	FoJsonBaseType::convert(embed);
	_varname = FoJsonUtils::gen_name(embed, _varname, _orig_varname);
	BESDEBUG( "fonc", "FoJsonArray::convert - converting array "
			<< _varname << endl );

	_array_type = FoJsonUtils::get_nc_type(_a->var());
	...
	for (; di != de; di++) {
		...
		BESDEBUG( "fonc", "FoJsonArray::convert - dimension size: "
				<< size << " chunksize: " <<
			...
			// shared dimension. Create it only once and share the FoJsonDim
		FoJsonDim *use_dim = find_dim(embed, _a->dimension_name(di), size);
	...
	if (_array_type == NC_CHAR) {
		...
		FoJsonDim *use_dim = find_dim(empty_embed, lendim_name, max_length, true);
	...
	if (!FoJsonGrid::InGrid && _actual_ndims == 1 && _a->name() == _a->dimension_name(_a->dim_begin())) {
		// is it already in there?
		FoJsonMap *map = FoJsonGrid::InMaps(_a);
		if (!map) {
			// This memory is leaked. jhrg 8/28/13
			FoJsonMap *new_map = new FoJsonMap(this);
			_grid_maps.push_back(new_map);		// save it here so we can free it later. jhrg 8/28/13
			FoJsonGrid::Maps.push_back(new_map);
		}
		else {
			_dont_use_it = true;
		}
	...
	BESDEBUG( "fonc", "FoJsonArray::convert - done converting array "
	 ...
	 * @returns either a new instance of FoJsonDim, or a shared FoJsonDim
	 * instance
	 * @throws BESInternalError if the name of a dimension is the same, but
	 * the size is different
	 */
FoJsonDim *
FoJsonArray::find_dim(vector<string> &embed, const string &name, int size, bool ignore_size)
{
	string oname;
	string ename = FoJsonUtils::gen_name(embed, name, oname);
	FoJsonDim *ret_dim = 0;
	vector<FoJsonDim *>::iterator i = FoJsonArray::Dimensions.begin();
	vector<FoJsonDim *>::iterator e = FoJsonArray::Dimensions.end();
	for (; i != e && !ret_dim; i++) {
	...
	if (!ret_dim) {
		ret_dim = new FoJsonDim(name, size);
		FoJsonArray::Dimensions.push_back(ret_dim);
	}
	else {
		ret_dim->incref();
	}
 ...
 */
void FoJsonArray::define(int ncid)
{
	BESDEBUG( "fonc", "FoJsonArray::define - defining array "
			<< _varname << endl );
		...
		if( !_defined && !_dont_use_it )
		{
			...
			for(; i != e; i++ )
			{
				...
				BESDEBUG( "fonc", "FoJsonArray::dim_id "
			...
			FoJsonAttributes::add_attributes( ncid, _varid, _a );
			FoJsonAttributes::add_original_name( ncid, _varid,
		...
		BESDEBUG( "fonc", "FoJsonArray::define - done defining array "
				<< _varname << endl );
	}
	 ...
	 * to the netcdf file
	 */
void FoJsonArray::write(int ncid)
{
	BESDEBUG( "fonc", "FoJsonArray::write for var " << _varname << endl );
				...
				switch( _array_type )
				{
					case NC_BYTE:
					{
						...
						if( stax != NC_NOERR )
						{
							...
							+ _varname;
							FoJsonUtils::handle_error( stax, err,
									__FILE__, __LINE__ );
						}
						delete [] data;
					}
		...
		{
			BESDEBUG( "fonc", "FoJsonTransform::    not using variable "
					<< _varname << endl );
		}

		BESDEBUG( "fonc", "FoJsonTransform::write_array done for "
				<< _varname << endl );
	}
 ...
 */
void FoJsonArray::dump(ostream &strm) const
{
	strm << BESIndent::LMarg << "FoJsonArray::dump - (" << (void *) this << ")" << endl;
	BESIndent::Indent();
	...
	if (_dims.size()) {
		...
		BESIndent::Indent();
		vector<FoJsonDim *>::const_iterator i = _dims.begin();
		vector<FoJsonDim *>::const_iterator e = _dims.end();
		for (; i != e; i++) {
			(*i)->dump(strm);
		}
		BESIndent::UnIndent();
