...
/** @brief Constructor for FoJsonMap that takes an array as the map
 *
 ...
 *
 * @param a A FoJsonArray representing the grid map
 * @param ingrid true if the FoJsonArray was created in the context of a
 * grid, false otherwise
 */
FoJsonMap::FoJsonMap( FoJsonArray *a, bool ingrid )
 ...
 *
 * If the FoJsonArray was created within the context of the grid, then the
 * FoJsonArray is owned by the map and can be deleted. Otherwise, it was
 ...
 *
 * All arrays are saved as a FoJsonMap if the array has only one dimension
 ...
 */
bool
FoJsonMap::compare( Array *tomap )
{
    bool isequal = true ;

    Array *map = _arr->array() ;

    BESDEBUG( "fonc", "FoJsonMap::compare - comparing " << tomap->name()
	      << " to " << map->name() << endl ) ;
    ...
    BESDEBUG( "fonc", "FoJsonMap::compare - done comparing " << tomap->name()
 ...
 */
void
FoJsonMap::add_grid( const string &name )
{
    _shared_by.push_back( name ) ;
}

/** @brief clear the embedded names for the FoJsonArray kept by this
 * instance
 */
void
FoJsonMap::clear_embedded()
{
    _arr->clear_embedded() ;
}
 ...
 * calling write on the FoJsonArray
 *
 * @param ncid The id of the netcdf file
 */
void
FoJsonMap::write( int ncid )
{
    _arr->write( ncid ) ;
}
 ...
 * including the FoJsonArray instance kept by this map.
 *
 * @param strm C++ i/o stream to dump the information to
 */
void
FoJsonMap::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonMap::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
