 ...
 */
FoJsonDouble::FoJsonDouble( BaseType *b )
    : FoJsonBaseType(), _f( 0 )
{
    _f = dynamic_cast<Float64 *>(b) ;
    if( !_f )
    {
	string s = (string)"File out netcdf, FoJsonDouble was passed a "
 ...
 */
FoJsonDouble::~FoJsonDouble()
{
}

/** @brief define the DAP Float64 in the netcdf file
 *
 * The parent class, FoJsonBaseType, actually defines the variable since
 ...
 */
void
FoJsonDouble::write( int ncid )
{
    BESDEBUG( "fonc", "FoJsonDouble::write for var " << _varname << endl ) ;
    size_t var_index[] = {0} ;
    ...
    delete data ;
    BESDEBUG( "fonc", "FoJsonDouble::done write for var " << _varname << endl ) ;
}
 ...
 */
void
FoJsonDouble::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonDouble::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
