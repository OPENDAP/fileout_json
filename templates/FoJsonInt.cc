 ...
 */
FoJsonInt::FoJsonInt( BaseType *b )
    : FoJsonBaseType(), _bt( b )
{
    ...
    if( !i32 && !u32 )
    {
	string s = (string)"File out netcdf, FoJsonInt was passed a "
 ...
 *
 * The DAP Int32 or UInt32 instance does not belong to the FoJsonByte
 ...
 */
void
FoJsonInt::write( int ncid )
{
    BESDEBUG( "fonc", "FoJsonInt::write for var " << _varname << endl ) ;
    size_t var_index[] = {0} ;
    int *data = new int ;
    ...
    delete data ;
    BESDEBUG( "fonc", "FoJsonInt::done write for var " << _varname << endl ) ;
}
 ...
 */
void
FoJsonInt::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonInt::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
