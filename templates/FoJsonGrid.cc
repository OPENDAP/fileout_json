 ...
 * different grids
 */
vector<FoJsonMap *> FoJsonGrid::Maps ;

/** @brief tells whether we are converting or defining a grid.
 *
 * This is used by FoJsonArray to tell if any single dimension arrays
 ...
 */
bool FoJsonGrid::InGrid = false ;

/** @brief Constructor for FoJsonGrid that takes a DAP Grid
 *
 ...
 */
FoJsonGrid::FoJsonGrid( BaseType *b )
    : FoJsonBaseType(), _grid( 0 ), _arr( 0 )
{
    _grid = dynamic_cast<Grid *>(b) ;
    if( !_grid )
    {
	string s = (string)"File out netcdf, FoJsonGrid was passed a "
 ...
 *
 * The DAP Grid instance does not belong to the FoJsonGrid instance, so it
 * is not deleted.
 *
 * Since maps can be shared by grids, FoJsonMap uses reference counting.
 * So instead of deleting the FoJsonMap instance, its reference count is
 * decremented.
 */
FoJsonGrid::~FoJsonGrid()
{
#if 0
	...
	while( !done )
	{
		vector<FoJsonMap *>::iterator i = _maps.begin();
		vector<FoJsonMap *>::iterator e = _maps.end();
		if( i == e )
		{
			done = true;
		}
 ...
 * The convert method creates a FoJsonMap for each of the grid's maps,
 * and a FoJsonArray for the grid's array.
 *
 ...
 */
void
FoJsonGrid::convert( vector<string> embed )
{
    FoJsonGrid::InGrid = true ;
    FoJsonBaseType::convert( embed ) ;
    _varname = FoJsonUtils::gen_name( embed, _varname, _orig_varname ) ;
    BESDEBUG( "fonc", "FoJsonGrid::convert - converting grid "
                      << _varname << endl ) ;
	...
	FoJsonMap *map_found = FoJsonGrid::InMaps( map ) ;

	// if we didn't find a match then found is still false. Add the
	// map to the vector of maps. If they are the same then create a
	// new FoJsonMap, add the grid name to the shared list and add the
	// FoJsonMap to the FoJsonGrid.
	if( !map_found )
	{
	    FoJsonArray *fa = new FoJsonArray( map ) ;
	    fa->convert( map_embed ) ;
	    map_found = new FoJsonMap( fa, true );
	    FoJsonGrid::Maps.push_back( map_found ) ;
	}
	else
	{
    ...
    _arr = new FoJsonArray( _grid->get_array() ) ;
    _arr->convert( _embed ) ;

    BESDEBUG( "fonc", "FoJsonGrid::convert - done converting grid "
                      << _varname << endl ) ;
    FoJsonGrid::InGrid = false ;
}
 ...
 * Included is each FoJsonMap instance as well as the FoJsonArray
 * representing the grid's array.
 *
 * @param strm C++ i/o stream to dump the information to
 */
void
FoJsonGrid::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonGrid::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
	...
	strm << endl ;
	BESIndent::Indent() ;
	vector<FoJsonMap *>::const_iterator i = _maps.begin() ;
	vector<FoJsonMap *>::const_iterator e = _maps.end() ;
	for( ; i != e; i++ )
	{
	    FoJsonMap *m = (*i) ;
	    m->dump( strm ) ;
...
FoJsonMap *
FoJsonGrid::InMaps( Array *array )
{
    bool found = false ;
    vector<FoJsonMap *>::iterator vi = FoJsonGrid::Maps.begin() ;
    vector<FoJsonMap *>::iterator ve = FoJsonGrid::Maps.end() ;
    FoJsonMap *map_found = 0 ;
    for( ; vi != ve && !found; vi++ )
    {
	map_found = (*vi) ;
	if( !map_found )
