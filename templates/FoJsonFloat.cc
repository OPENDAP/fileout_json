...
/** @brief Constructor for FoJsonFloat that takes a DAP Float32
 *
 ...
 */
FoJsonFloat::FoJsonFloat( BaseType *b )
    : FoJsonBaseType(), _f( 0 )
{
    _f = dynamic_cast<Float32 *>(b) ;
    if( !_f )
    {
	string s = (string)"File out netcdf, FoJsonFloat was passed a "
 ...
 *
 * The DAP Float32 instance does not belong to the FoJsonFloat instance, so it
 * is not deleted up.
 */
FoJsonFloat::~FoJsonFloat()
{
}

/** @brief define the DAP Float32 in the netcdf file
 *
 * The definition actually takes place in FoJsonBaseType. This function
 ...
 */
void
FoJsonFloat::define( int ncid )
{
    FoJsonBaseType::define( ncid ) ;

    if( !_defined )
    {
	FoJsonAttributes::add_attributes( ncid, _varid, _f ) ;
 ...
 */
void
FoJsonFloat::write( int ncid )
{
    BESDEBUG( "fonc", "FoJsonFloat::write for var " << _varname << endl ) ;
    size_t var_index[] = {0} ;
    ...
    delete data ;
    BESDEBUG( "fonc", "FoJsonFloat::done write for var " << _varname << endl ) ;
}
 ...
 */
void
FoJsonFloat::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonFloat::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
