...
/** @brief Constructor for FoJsonByte that takes a DAP Byte
 *
 ...
 */
FoJsonByte::FoJsonByte( BaseType *b )
    : FoJsonBaseType(), _b( 0 )
{
    _b = dynamic_cast<Byte *>(b) ;
    if( !_b )
    {
	string s = (string)"File out netcdf, FoJsonByte was passed a "
 ...
 *
 * The DAP Byte instance does not belong to the FoJsonByte instance, so it
 * is not deleted.
 */
FoJsonByte::~FoJsonByte()
{
}
 ...
 */
void
FoJsonByte::define( int ncid )
{
    FoJsonBaseType::define( ncid ) ;

    if( !_defined )
    {
	FoJsonAttributes::add_attributes( ncid, _varid, _b ) ;
 ...
 */
void
FoJsonByte::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonByte::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
