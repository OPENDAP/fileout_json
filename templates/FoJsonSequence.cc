...
/** @brief Constructor for FoJsonSequence that takes a DAP Sequence
 *
 ...
 */
FoJsonSequence::FoJsonSequence( BaseType *b )
    : FoJsonBaseType(), _s( 0 )
{
    _s = dynamic_cast<Sequence *>(b) ;
    if( !_s )
    {
	string s = (string)"File out netcdf, FoJsonSequence was passed a "
 ...
 *
 * The DAP Sequence instance does not belong to the FoJsonSequence
 * instance, so it is not delete it.
 */
FoJsonSequence::~FoJsonSequence()
{
}
 ...
 */
void
FoJsonSequence::convert( vector<string> embed )
{
 ...
 */
void
FoJsonSequence::define( int ncid )
{
 ...
 */
void
FoJsonSequence::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonSequence::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
