...
/** @brief Constructor for FoJsonStr that takes a DAP Str
 *
 ...
 */
FoJsonStr::FoJsonStr( BaseType *b )
    : FoJsonBaseType(), _str( 0 ), _dimid( 0 ), _data( 0 )
{
    _str = dynamic_cast<Str *>(b) ;
    if( !_str )
    {
	string s = (string)"File out netcdf, FoJsonStr was passed a "
 ...
 *
 * The DAP Str instance does not belong to the FoJsonByte instance, so it
 ...
 */
void
FoJsonStr::define( int ncid )
{
    if( !_defined )
    {
	BESDEBUG( "fonc", "FoJsonStr::define - defining "
			  << _varname << endl ) ;
	...
	FoJsonAttributes::add_attributes( ncid, _varid, _str ) ;
	FoJsonAttributes::add_original_name( ncid, _varid,
					   _varname, _orig_varname ) ;

	BESDEBUG( "fonc", "FoJsonStr::define - done defining "
			  << _varname << endl ) ;
    }
}
 ...
 */
void
FoJsonStr::write( int ncid )
{
    BESDEBUG( "fonc", "FoJsonStr::write for var " << _varname << endl ) ;
    ...
    delete _data ;
    _data = 0 ;
    BESDEBUG( "fonc", "FoJsonStr::done write for var " << _varname << endl ) ;
}
 ...
 */
void
FoJsonStr::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonStr::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
