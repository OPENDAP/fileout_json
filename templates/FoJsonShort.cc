 ...
 */
FoJsonShort::FoJsonShort( BaseType *b )
    : FoJsonBaseType(), _bt( b )
{
    Int16 *i16 = dynamic_cast<Int16 *>(b) ;
    UInt16 *u16 = dynamic_cast<UInt16 *>(b) ;
    if( !i16 && !u16 )
    {
	string s = (string)"File out netcdf, FoJsonShort was passed a "
 ...
 *
 * The DAP Int16 or UInt16 instance does not belong to the FoJsonByte
 * instance, so it is not deleted.
 */
FoJsonShort::~FoJsonShort()
{
}
 ...
 */
void
FoJsonShort::define( int ncid )
{
    FoJsonBaseType::define( ncid ) ;

    if( !_defined )
    {
	FoJsonAttributes::add_attributes( ncid, _varid, _bt ) ;
 ...
 */
void
FoJsonShort::write( int ncid )
{
    BESDEBUG( "fonc", "FoJsonShort::write for var " << _varname << endl ) ;
    size_t var_index[] = {0} ;
    ...
    delete data ;
    BESDEBUG( "fonc", "FoJsonShort::done write for var " << _varname << endl ) ;
}
 ...
 */
void
FoJsonShort::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonShort::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
