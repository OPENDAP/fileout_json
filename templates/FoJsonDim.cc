 ...
 */
FoJsonDim::FoJsonDim( const string &name, int size )
...
FoJsonDim::define( int ncid )
{
	...
	if( _name.empty() )
	{
	    ...
	    dimname_strm << "dim" << FoJsonDim::DimNameNum+1 ;
	    FoJsonDim::DimNameNum++ ;
	    _name = dimname_strm.str() ;
	}
	else
	{
	    _name = FoJsonUtils::id2netcdf( _name ) ;
	}
 ...
 */
void
FoJsonDim::dump( ostream &strm ) const
{
    strm << BESIndent::LMarg << "FoJsonDim::dump - ("
			     << (void *)this << ")" << endl ;
    BESIndent::Indent() ;
